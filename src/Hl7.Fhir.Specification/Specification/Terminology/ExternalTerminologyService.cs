...
namespace Hl7.Fhir.Specification.Terminology
{
    public class ExternalTerminologyService : ITerminologyService
    {
        ...
        {
            if (string.IsNullOrEmpty(id))
                return Endpoint.TypeOperation(RestOperation.TRANSLATE, FHIRAllTypes.CodeSystem.GetLiteral(), parameters, useGet) as Parameters;
            else
                return Endpoint.InstanceOperation(new Uri($"{FHIRAllTypes.CodeSystem.GetLiteral()}/{id}", UriKind.Relative), RestOperation.TRANSLATE, parameters, useGet) as Parameters;
        }
