...
namespace Hl7.Fhir.Specification.Tests
{
    public class TerminologyTests : IClassFixture<ValidationFixture>
    {
        ...
        public void ExternalServiceTranslateSimpleTranslate()
        {
            ...
            var result = svc.Translate(parameters);
        ...
        public void ExternalServiceLookupPropertiesDisplayAndInactiveStatus()
        {
            ...
            Assert.Collection(result.Parameter, 
                parameter =>
                {
                    Assert.Equal("display", parameter.Name);
                    Assert.IsType<FhirString>(parameter.Value);
                    Assert.Equal("teriparatide 20 microgram injection, 2.4 mL cartridge", ((FhirString)parameter.Value).Value);
                },
                parameter =>
                {
                    Assert.Equal("property", parameter.Name);
